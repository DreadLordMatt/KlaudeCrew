...
async def test_empty_path_returns_empty():
    ...
    req = _req("")
    with patch("kiro_crew.dashboard.handlers.files.browse._sel", return_value=_mock_sel()):
...
async def test_sensitive_path_returns_403():
    ...
    with patch("kiro_crew.dashboard.handlers.files.browse.is_sensitive_path", return_value=True), \
         patch("kiro_crew.dashboard.handlers.files.browse.os.path.isfile", return_value=True), \
         patch("kiro_crew.dashboard.handlers.files.browse._sel", return_value=_mock_sel()):
        resp = await api_file_diff(req)
...
async def test_textconv_hardening(tmp_path):
    ...
    with patch("kiro_crew.dashboard.handlers.files.browse._sel", return_value=_mock_sel()), \
         patch("kiro_crew.dashboard.handlers.files.browse.subprocess.run", side_effect=spy_run):
        req = _req(str(f))
        resp = await api_file_diff(req)
...
async def test_timeout_returns_not_git(tmp_path):
    ...
    with patch("kiro_crew.dashboard.handlers.files.browse._sel", return_value=_mock_sel()), \
         patch("kiro_crew.dashboard.handlers.files.browse.subprocess.run", side_effect=timeout_run), \
...
async def test_sel_audit_logging_on_success(tmp_path):
    ...
    mock_sel = _mock_sel()
    with patch("kiro_crew.dashboard.handlers.files.browse._sel", return_value=mock_sel), \
         patch("kiro_crew.dashboard.handlers.files.browse.is_sensitive_path", return_value=False), \
         patch("kiro_crew.dashboard.handlers.files.browse.subprocess.run", side_effect=subprocess.CalledProcessError(1, "git")):
        req = _req(str(f))
        await api_file_diff(req)
