...
def test_inbound_ungoverned_permit_audited_best_effort(monkeypatch, tmp_path) -> None:
    # GPT round-8: the codebase invariant is "audit every permission decision"
    # (security-deep-dive). An ungoverned inbound ALLOW (default OSS build, no
    # policy) IS audited — best-effort (critical is not set), so it records the
    # forensic trail without gating availability on SEL disk health.
    from kiro_crew.platform import governance_profiles as gp
    ...
    try:
        ...
        assert rec["item"] == "discord"
        # Ungoverned allow is best-effort, NOT critical (unlike a governed allow).
        assert not rec.get("critical", False)
    finally:
        gp.reset_store()
