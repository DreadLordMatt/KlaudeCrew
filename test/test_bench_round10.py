...
def test_an_ordinary_write_still_truncates(tmp_path: Path) -> None:
    """O_TRUNC moved after the check, so the truncation must still happen.

    Without this, a stale longer file would leave trailing bytes past the new content
    and the report would be corrupt JSON.
    """
    ...
    with os.fdopen(fd, "w", encoding="utf-8") as fh:
        fh.write("{}")
    assert target.read_text(encoding="utf-8") == "{}", "the file was not truncated"
