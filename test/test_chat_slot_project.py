...
class TestChatSlotProject:
    ...
    async def test_set_project(self, tmp_path):
        ...
        with patch("kiro_crew.dashboard.chat_handlers_config._save_recent_project"):
    ...
    async def test_sensitive_path_returns_403(self, tmp_path):
        slot = _ChatSlot("test")
        state = _mock_state(slot)
        with patch("kiro_crew.dashboard.chat_handlers_config.is_sensitive_path", return_value=True):
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post(
                    "/api/chat/slots/test/project",
                    json={"project": str(tmp_path)},
