...
from kiro_crew.eval.bench.safepath import UnsafePathError, open_write_nofollow
...
def test_overwriting_an_existing_plain_file_still_works(
    ...
    """Re-running a benchmark with the same --stem must not be refused.

    A guard that rejected any existing file would be stronger against links and
    would also break the ordinary case, so the distinction is asserted.
    """
    target = tmp_path / "report.json"
