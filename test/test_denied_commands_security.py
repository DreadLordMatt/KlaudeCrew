...
class TestCatalog:
    ...
    def test_token_mint_is_blocked_in_both_the_cli_and_module_forms(self):
        ...
        for allowed in (
            ...
            # A `-c` payload that imports the package but does NOT mint: the verb, not the
            # import, is what this rule blocks. Pins that the `-c` fix narrowed nothing.
            "python -c 'import kiro_crew.cli' && echo ok",
            'python -c "from kiro_crew.cli import main; main()" doctor',
            'python -c "import kiro_crew; print(kiro_crew.__version__)"',
        ):
