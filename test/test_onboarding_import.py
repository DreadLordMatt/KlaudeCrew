...
class TestApply:
    ...
    def test_mcp_import_uses_the_shared_dashboard_lock(
        ...
        assert api._write_mcp(item, tmp_path / "destination") == "imported"
        assert entered == [True]
    ...
    def test_skill_write_failure_removes_partial_package_and_skips_ledger(
        ...
        real_atomic_write = api.atomic_write
        package_writes = 0

        def fail_second_package_write(path: Path | str, content: str, **kwargs: object) -> None:
        ...
        monkeypatch.setattr(api, "atomic_write", fail_second_package_write)
        plan = _select(
            api.preview_import(home=tmp_path / "home", env={}),
            ("claude_code", "skills"),
        )
