...
class TestHandleCronListNextRun:
    ...
    def test_includes_next_run(self, cron_service: CronService) -> None:
        ...
        now = time.time()
        with patch("kiro_crew.slack.keyword_commands.compute_next_run_ts", return_value=now + 7200):
    ...
    def test_no_next_run_for_disabled(self, cron_service: CronService) -> None:
        ...
        with patch("kiro_crew.slack.keyword_commands.compute_next_run_ts", return_value=None):
            result = _handle_cron_command("cron list", cron_service, "C123", "t123")
        assert result is not None
    ...
    def test_next_run_days(self, cron_service: CronService) -> None:
        cron_service._jobs = [_make_job()]
        now = time.time()
        with patch("kiro_crew.slack.keyword_commands.compute_next_run_ts", return_value=now + 3 * 86400 + 7200):
            result = _handle_cron_command("cron list", cron_service, "C123", "t123")
        assert result is not None
    ...
    def test_next_run_minutes(self, cron_service: CronService) -> None:
        cron_service._jobs = [_make_job()]
        now = time.time()
        with patch("kiro_crew.slack.keyword_commands.compute_next_run_ts", return_value=now + 1800):
            result = _handle_cron_command("cron list", cron_service, "C123", "t123")
        assert result is not None
        assert "⏭ in" in result
    ...
    def test_next_run_less_than_one_minute(self, cron_service: CronService) -> None:
        cron_service._jobs = [_make_job()]
        now = time.time()
        with patch("kiro_crew.slack.keyword_commands.compute_next_run_ts", return_value=now + 30):
            result = _handle_cron_command("cron list", cron_service, "C123", "t123")
        assert result is not None
    ...
    def test_next_run_past_due(self, cron_service: CronService) -> None:
        cron_service._jobs = [_make_job()]
        now = time.time()
        with patch("kiro_crew.slack.keyword_commands.compute_next_run_ts", return_value=now - 5):
            result = _handle_cron_command("cron list", cron_service, "C123", "t123")
        assert result is not None
    ...
    def test_message_is_redacted(self, cron_service: CronService) -> None:
        ...
        with patch("kiro_crew.slack.keyword_commands.compute_next_run_ts", return_value=None), \
             patch("kiro_crew.slack.keyword_commands.redact_exfiltration_urls",
                   return_value=("[URL_REDACTED]", True)) as mock_url, \
             patch("kiro_crew.slack.keyword_commands.redact_credentials",
    ...
    def test_remove_all_redacts_name(self, cron_service: CronService) -> None:
        ...
        cron_service._jobs = [job]
        with patch("kiro_crew.slack.keyword_commands.redact_exfiltration_urls",
                   return_value=("[URL_REDACTED]", True)) as mock_url, \
             patch("kiro_crew.slack.keyword_commands.redact_credentials",
                   return_value=("[REDACTED]", True)) as mock_cred:
