...
class TestDeliveryFailureKeepsTheResult:
    ...
    def test_drain_keeps_what_it_could_not_deliver(self) -> None:
        orch = _orch()
        slot = _slot()
        st = orch._store("s1")
        st.add_forward("first")
        st.add_forward("second")
        # First delivers, second refuses: exactly one must remain.
        with patch.object(orch, "_post", side_effect=[True, False]):
...
class TestReviewFixes:
    ...
    async def test_reconcile_redelivers_orphaned_forwards(self) -> None:
        ...
        orch = _orch(state=state)
        with patch.object(orch, "_post") as post:
            orch._store("s1")  # triggers _reconcile -> synchronous drain
        post.assert_called_once()
