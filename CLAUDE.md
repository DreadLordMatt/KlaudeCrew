...
- **Two agents, two sets of instruction files — do not confuse them.** *You*
  ...
  this repo.
- **Where you are.** Never work on `main`. Local `main` **tracks
  `upstream/main`** (kirodotdev/KiroCrew), so a bare `git push` from `main`
  would target the upstream project — the hook blocks it. Work happens on
  `feat/<issue>-<slug>` / `fix/<issue>-<slug>` branches off `main`.
...
| Action | Rule |
|---|---|
| Create/switch feature branch off `main` | Free |
...
| Push to `main`, to `upstream`, `--force*`, `+refspec`, remote delete, `--all/--mirror/--tags`, tags | **Never** (hook-enforced, best-effort) |
...
until proven otherwise on a clean `origin/main` checkout.
