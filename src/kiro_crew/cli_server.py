...
from kiro_crew.instances.discovery import (  # noqa: F401
...
def _warn_ambiguous_gateways(ports: list[int]) -> None:
    """Tell the user which gateways were found when discovery refuses to pick one."""
    print(
        f"⚠️  Multiple gateways are running (ports {', '.join(str(p) for p in ports)}); "
...
def resolve_client_port(cli_port: int | None) -> int:
    ...
    if cfg_port:
        return cfg_port
    discovered = _marker_port(on_ambiguous=_warn_ambiguous_gateways)
    if discovered:
        return discovered
