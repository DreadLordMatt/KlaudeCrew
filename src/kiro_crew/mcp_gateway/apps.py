...
def visibility_allows(tool: Any, audience: str) -> VisibilityVerdict:
    ...
    * ``_meta`` or ``ui`` present but NOT a dict → the container that would hold
      the declaration is unreadable, so a visibility may well be in there and we
      cannot see it → deny, flagged ``unreadable``.
