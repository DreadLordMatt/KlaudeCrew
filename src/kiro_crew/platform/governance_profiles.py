...
class ProfileStore:
    ...
    def _ensure_fresh(self) -> None:
        ...
        The discipline is chosen by ONE condition — has this store ever loaded? —
        not by which thread is calling:

        * **Unprimed** (``_snap.loaded`` is False): the first load may not be
          skipped, so a caller that finds the lock held WAITS for it. Serving an
          empty never-loaded snapshot would fail OPEN (see ``_Snapshot.loaded``).
          This can happen at most once per process.
        * **Warm** (already loaded): NON-BLOCKING. A caller that can't grab the
          lock immediately serves the current immutable snapshot and returns. It
          must never block here, because this path is reachable on the event loop
          (the synchronous PreToolUse gate) where waiting on another thread's
          filesystem I/O would wedge the loop. That is safe because ``_snap`` is
          only ever REPLACED wholesale (an atomic ref swap): a concurrent reader
          sees a coherent prior-or-next snapshot, and since a prior snapshot
          exists, the worst case is authorizing against the last committed state
          for ONE call — the next access self-heals.
        ...
        per-thread "always block" discipline for off-loop callers on a WARM store.
        On a warm store its only benefit is closing a staleness window one call
        wide while a reload is concurrently in flight, which is not worth a
        thread-local plus a dual code path — and it invites someone to later reach
        for the blocking path from the loop, reintroducing the wedge. A surface
        that needs strict read-your-writes should add it deliberately, with tests.
        ...
        if self._snap.loaded:
            ...
            if fp == self._fingerprint:
                return
        # The FIRST load may NOT be skipped. A never-loaded snapshot is EMPTY, and an
        # empty snapshot is indistinguishable from a genuine "no profiles configured"
        # host: the caller would resolve ``profile=None`` and ``governance_permits``
        # would hand back its ``ungoverned`` default-PERMIT — a fail-OPEN on a host
        # that does have restrictive profiles, even for a ``fail_closed=True`` caller
        # (the default-permit is a normal return, not an exception, so fail_closed
        # never engages). Concurrent first-touch is the EXPECTED case, not an exotic
        # one: an unprimed store plus a startup burst across the transports puts
        # several threads here at once. So on an unprimed store we WAIT for the
        # in-flight first load instead of serving nothing.
        #
        # Waiting here cannot wedge the event loop: it happens only while
        # ``loaded`` is False, i.e. once per process, and the holder is doing the
        # very load this caller needs. Every later call takes the non-blocking path.
        blocking = fp is None  # i.e. the store was unprimed when we checked
        if not self._lock.acquire(blocking=blocking):
            return  # warm store: another thread owns this reload; serve the snapshot
        try:
            # A caller that WAITED for the lock may have waited while the winner
            # published, so its view is stale and it must re-stat; a warm caller that
            # took the lock without waiting (``acquire(blocking=False)`` succeeded)
            # still holds a current pre-lock fingerprint and REUSES it — re-statting
            # would run a second iterdir+stat walk on the event loop (a slow-FS
            # stall) for no freshness gain. Either way the value used for the
            # freshness test is the same one committed below, so the committed
            # fingerprint always describes the snapshot actually published.
            if blocking:
                fp = _dir_fingerprint(directory)
            if self._snap.loaded and fp == self._fingerprint:
                return  # already fresh (another thread reloaded, or unchanged)
