...
this seam. ``register_acp_backends`` is the one real override: it attaches
the two hook methods ``AcpClient`` already looks up via ``getattr`` at spawn
time (``mcp_servers.claude_session_servers`` and
``settings_seed.write_claude_local_settings``) onto the class, exactly as
``platform/defaults.py``'s own docstring for ``DefaultProviderRegistry``
describes an "internal companion" doing.
"""
