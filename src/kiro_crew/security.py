...
def _build_sensitive_regex() -> re.Pattern[str]:
    ...
    # home-relative fenced path.
    userprofile = rf"(?:{re.escape('%USERPROFILE%')}|{re.escape('$env:USERPROFILE')})"
    win_home_alts = (
    ...
    # by their remainder.
    appdata_var = rf"(?:{re.escape('%APPDATA%')}|{re.escape('$env:APPDATA')})"
...
def _is_path_like(token: str) -> bool:
    ...
    # on Windows hosts. Matches are limited LEXICALLY to the drive (or UNC
    # share) containing ``Path.home()``: every fenced target lives under home,
    # so a foreign-drive token can never match — and routing it onward would
    # only feed ``os.path.realpath`` a disconnected mapped drive or dead UNC
    # host, a synchronous network stall on the caller. The comparison is pure
    # string work (no ``os.path.splitdrive``, which is OS-specific), so on
    # POSIX hosts — where home has no drive — these shapes simply never match.
    if _WIN_DRIVE_PATH_RE.match(token):
        home_str = str(Path.home())
        if len(home_str) >= 3 and home_str[1] == ":" and home_str[2] in "\\/":
            return token[:2].casefold() == home_str[:2].casefold()
        return False
    if token.startswith("\\\\"):
        home_str = str(Path.home())
        if home_str.startswith("\\\\"):
            ...
            return token_cf == share or token_cf.startswith((share + "\\", share + "/"))
        return False
