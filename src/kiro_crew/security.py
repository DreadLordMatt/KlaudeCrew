    ...
    for tokens in _self_token_frames(text_lower):
        ...
        for i, token in enumerate(tokens):
            ...
            if _data_consumer_exempt(i, token, programs, tokens):
                continue
            # AN INLINE PROGRAM THAT IMPORTS OUR CLI IS DENIED WITHOUT NEEDING THE VERB.
            #
            # Everywhere else the verb is the trigger, because ``kirocrew doctor`` is
            # legitimate and only ``kirocrew token`` mints. That reasoning does not survive
            # ``-c``: the payload is arbitrary Python running with the interpreter's full
            # authority, so it can BUILD the verb rather than pass it —
            # ``python -c "import sys; sys.argv.append('token'); from kiro_crew.cli import
            # main; main()"`` names no ``token`` argv word at all, and neither does
            # ``main(['token'])``. Requiring the verb in argv is therefore not enforceable on
            # this path; the honest gate is the import. Found in review (GPT 5.6).
            #
            # Scoped tightly: only a ``-c``/``-c<payload>`` operand that matches
            # ``_SELF_IMPORT_RE``. ``python -c "print(1)"`` is untouched, and so is every
            # payload that does not reach for this package.
