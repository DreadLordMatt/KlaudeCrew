...
def _is_self_module_invocation(tokens: list[str], i: int) -> bool:
    """True if ``tokens[i]`` is a ``python`` that runs the product AS A MODULE.
    ...
    have their operand skipped. The first version of this stopped at the first token that did
    ...
    deeper. Found in review. Modelling which flags consume an operand is the fix; "stop at the
    ...
    for later in tokens[i + 1 :]:
        ...
        if len(stripped) > 2 and stripped[:2] in _PYTHON_OPERAND_FLAGS:
            continue
        # Only interpreter FLAGS may sit between; anything else means this is not the
        # `-m <product>` shape (`python script.py`, `python -c "..."`).
