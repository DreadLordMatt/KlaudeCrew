...
- **Do NOT hardcode a port.** `kirocrew token` prints the URL of *this* instance.
  ...
  `{"error": "Token required"}` — observed 65 times in one day, against an instance
...
The internal ops workflow this app is modeled on kept fix patterns in a table with
confidence and trust, and that table is what let a new responder skip hours of
rediscovery. What makes an entry useful:
...
- **Fix** is specific enough to act on: the actual parameter, the actual command
  shape, the actual config key. "Increase memory" is not a fix; "count_records
  reads the whole CSV into pandas — bump 512→1024MB, long term use the validation
  result's record count" is.
