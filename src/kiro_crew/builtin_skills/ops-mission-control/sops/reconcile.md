...
3. **Absence is not evidence.** A signal missing from `firing` means one of three
   ...
   **`ok` is necessary and not sufficient. Also require
   ...
   delivers by PUSH into a spool the poll drains — the inbound `webhook` source — where a
   signal appears in exactly one cycle's result and is absent from every cycle afterwards
   whether or not the fault is still live. So absence there is the normal steady state,
