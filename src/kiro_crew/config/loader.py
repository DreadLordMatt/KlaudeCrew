...
class AgentConfig:
    ...
    session_control: bool = field(
        ...
        metadata=_meta(
            "Session Control",
            "Let one chat session send a message to, stop, or read another session "
            "of yours. Delivery goes through the target's normal user-input path "
            "(steer, queue, or a fresh turn) and always names the sending session, "
            "so nothing arrives unattributed. Turn it off to make the three tools "
            "refuse. Sessions can only reach peers in the same workspace; incognito, "
