...
def write_report(
    ...
    _write_text_nofollow(safe_dir / f"{stem}.md", format_report(outcome), "markdown report")
...
def _write_text_nofollow(path: Path, text: str, what: str) -> None:
    """Write *text* to *path* refusing to follow a final symlink.

    Exists because ``Path.write_text`` on a guarded path is not enough: the guard
    returns the RESOLVED path, so a link planted at that name has already been
    followed by the time the write happens.
    """
    import os

    fd = open_write_nofollow(path, what=what)
...
def _measurable_count(metrics: dict, field: str, k: int) -> int | None:
    ...
    """
    raw = metrics.get(field, {}).get(str(k))
