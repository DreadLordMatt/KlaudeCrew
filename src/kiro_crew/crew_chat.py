...
class CrewOrchestrator:
    ...
    def _post(self, slot: Any, content: str, kind: str = "crew") -> bool:
        ...
        try:
            ...
            cls = "msg msg-a crew-reply" if kind in _ANSWER_KINDS else "msg msg-a"
            slot.append("assistant", content, cls, broadcast=False)
            self._state.broadcast_ws(
                "chat_message",
                {"slot": slot.key, "role": "assistant", "content": content,
                 # `cls` must ride the frame as well: the store reducer reads it
                 # from the payload, so omitting it made the marker work only
                 # AFTER a reload — a live crew answer was still collapsed into
                 # "Worked through N steps". Same value on both paths, one source.
                 "cls": cls, "kind": kind},
            )
        except Exception:
        ...
        try:
            append_if_absent_off_loop(
                ...
                "assistant",
                content,
                # The SAME class as the in-memory copy. Without it the marker
                # lived only in `slot.messages`, so it survived a page reload
                # (same process) but not a gateway restart, where the rows are
                # replayed from this log and earlier crew answers collapsed
                # back into "Worked through N steps".
                cls=cls,
            )
        except Exception:
    ...
    async def _queue_forward(self, slot: Any, body: str) -> None:
        ...
        if self._post(slot, body, kind="crew_result"):
            st.remove_forwards({fid})

    def _drain_forwards(self, slot: Any) -> None:
        ...
        for f in list(st.forwards):
            if self._post(slot, str(f.get("body", "")), kind="crew_result"):
