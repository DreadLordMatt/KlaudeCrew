...
grant authority, set `mode: "act"` and add a rule to the app's
`data/config.json` — deliberately *not* settable over the API, so a single API call
can never widen the app's own authority:
...
`resource_glob` is required — there is no wildcard — and `actions` narrows further,
so the rule above permits `ack` and still refuses `resolve` on the same incident.
