...
class TestACredentialBearingRemoteIsRefused(unittest.IsolatedAsyncioTestCase):
    ...
    def test_userinfo_detection_covers_the_shapes_that_matter(self):
        """scp-style and `ssh://git@` contain an `@` and must keep working."""
        for remote, refuse in (
            ("https://user:ghp_secret@github.com/org/repo.git", True),
            ("https://u:p@host:8443/r.git", True),
            ("https://ghp_secret@github.com/org/repo.git", False),  # username, no password
            ("git@github.com:org/repo.git", False),  # no `://`, so no userinfo
            ("ssh://git@github.com/org/repo.git", False),  # key auth, not a secret
...
class TestNoHandlerParsesAStoredFileOnTheLoop(unittest.IsolatedAsyncioTestCase):
    ...
    #: Helpers that read and parse a file whose size grows with use.
    FILE_PARSING = (
    ...
    def test_every_file_parsing_call_is_offloaded(self):
        import inspect
        import re
        ...
        for lineno, line in enumerate(source.splitlines(), 1):
            ...
            if stripped.startswith("#"):
                continue  # the comments explaining these fixes necessarily name the calls
            for helper in self.FILE_PARSING:
                if f"{helper}(" not in line:
                    continue
                # Accepted forms: `to_thread(helper)` and `to_thread(helper, arg)`. The call
                # must be PASSED to to_thread, not invoked as its argument — which is the
                # mistake that hid one of these before (`to_thread(f, g())` runs `g` inline).
                if re.search(rf"to_thread\(\s*{re.escape(helper)}\s*[,)]", line):
                    continue
                offenders.append(f"{lineno}: {stripped}")
        self.assertEqual(
            offenders,
            [],
            "these parse a growing file on the event loop — wrap with asyncio.to_thread:\n  "
            + "\n  ".join(offenders),
        )
