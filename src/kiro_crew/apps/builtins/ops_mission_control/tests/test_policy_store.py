...
class TestConcurrentWritesCannotRestoreAStaleCeiling(unittest.TestCase):
    ...
    def test_every_writer_holds_the_lock(self):
        ...
        over the SOURCE of each public writer so a fourth one added later without the lock
        ...
        for writer in (policy_store.set_mode, policy_store.set_rules, policy_store.put):
            with self.subTest(writer=writer.__name__):
                source = inspect.getsource(writer)
                self.assertIn(
                    "_PolicyLock()",
                    source,
                    f"{writer.__name__} does a read-modify-write without the lock",
                )
