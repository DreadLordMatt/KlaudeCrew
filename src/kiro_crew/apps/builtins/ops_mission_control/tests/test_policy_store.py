...
class TestOutboundDestinationsAreOperatorOnly(_HomeIsolated):
    ...
    def test_a_legacy_destination_is_migrated_out_of_config(self):
        import json
        ...
        cfg.write_text(
            json.dumps(
                {"ledger_sync_remote": "https://github.com/org/legacy.git", "region": "us-east-1"}
            ),
            encoding="utf-8",
        )
        # First read migrates it onto the fenced floor and clears the shadow copy.
        self.assertEqual(ledger_sync.remote(), "https://github.com/org/legacy.git")
        remaining = read_config()
        self.assertNotIn("ledger_sync_remote", remaining)
        self.assertEqual(remaining.get("region"), "us-east-1")
