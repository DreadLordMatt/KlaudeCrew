...
class _AdmissionPatch:
    """Patch the platform admission functions companion.py imports lazily."""

    def __init__(self, evaluator: Any) -> None:
        self._patches = [
            mock.patch("kiro_crew.platform.admission.evaluate_admission", evaluator),
            mock.patch("kiro_crew.platform.admission.load_admission_policy", lambda: mock.Mock()),
            mock.patch("kiro_crew.platform.admission.seed_default_policy", lambda: True),
        ]
