...
class TestOffShiftCannotWrite(_Env):
    ...
    def _grant_act(self, login: str) -> None:
        # The ceiling goes on the KEYSTONE floor, not into `config.json`: that file is
        # agent-writable and the read path deliberately ignores a ceiling found there (see
        # `policy_store` — honouring it let the constrained party set its own ceiling). Only
        # the provider login, which the app legitimately reads from config, stays in config.
