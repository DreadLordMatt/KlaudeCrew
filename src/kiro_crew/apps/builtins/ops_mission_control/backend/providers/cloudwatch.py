    ...
    def _poll_sync(self) -> list[Signal]:
        ...
        for state in states:
            ...
            try:
                ...
                # snapshot as a complete, healthy one: with `include_insufficient_data` on
                # this loop runs twice, so a failure on the second state silently truncated
                # the estate — and the caller cannot tell a short list from a quiet one.
                # `poll_all` needs the raise to record the source as unhealthy and arm its
                # backoff window; the operator then sees "cloudwatch did not answer" rather
                # than a board that looks calm.
