...
Each module here implements one or more of the four Protocols in ``base.py``. An
Amazon-internal companion ships its own adapters in its own package and registers
...
#: (documented KiroCrew behavior), so NOTHING sensitive may be stored here —
...
def write_config(payload: dict[str, Any]) -> None:
    ...
    auth (a documented KiroCrew behavior apps rely on to bootstrap their UI), so
