...
Hand-maintained handover documents are the common practice here, and they
was one of the most-used artifacts the team had: at shift change the incoming
responder does not want a list of every incident, they want the small number of things
that will actually page them, plus what to do about each.

That document took hours of upkeep to maintain and went stale between edits. Everything
in it that is *generic* is already data this app owns:

- **Recurring patterns ranked by frequency** — the source digest's core section, and
