...
def decide_proposal(incident_id: str, *, approve: bool, digest: str = "") -> dict[str, Any]:
    ...
    if state != PROPOSAL_PENDING:
        # Already decided. Idempotent-looking retries are common (a double-click, a
        # replayed request), so say what happened rather than pretending to act.
        return {
            "ok": False,
    ...
    if digest and stored_digest and digest != stored_digest:
        return {
            "ok": False,
            "reason": (
                "the proposal changed since you read it — re-read it and decide again "
    ...
    update_fields(incident_id, proposed_action=proposal)
...
def expire_stale_proposals(*, now: str = "") -> list[str]:
    ...
    for inc in read_index().values():
        ...
        update_fields(inc.incident_id, proposed_action=proposal)
