...
def app_mode() -> str:
    ...
    Read from the KEYSTONE policy store, not ``config.json``: the mode is a security ceiling
    and ``config.json`` is agent-writable (see ``policy_store``). ``migrate_from_config_if_
    needed`` lifts a pre-fix install's value onto the fenced floor on first read.
    """
    policy_store.migrate_from_config_if_needed()
...
def load_rules() -> list[AutonomyRule]:
    # Keystone store, same reasoning as ``app_mode``: an act-rule is half the authorization,
    # so it cannot live where the agent can write it.
    policy_store.migrate_from_config_if_needed()
    ...
    try:
        ...
        for src in sources:
            try:
                ...
                logger.debug(
                    "ops-mission-control: rotation source %r unavailable",
                    getattr(src, "id", src),
                    exc_info=True,
                )
                continue
...
def _shift_sync(src: Any) -> ShiftStatus | None:
    ...
    Every shipped source computes its answer synchronously and wraps that core in an async
    ``on_shift()`` (`schedule_file.resolve_now`, `pagerduty._on_shift_sync`). Preferring the
    sync core keeps this whole gate off the loop. A source that offers only the coroutine
    returns None — "did not answer" — rather than being run here: driving an event loop from
    inside a worker thread to poll one provider is worse than abstaining, and abstaining is
    already the fail-open behaviour documented above.
    """
