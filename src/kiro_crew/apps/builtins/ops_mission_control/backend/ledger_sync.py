...
def _credential_bearing_lines() -> list[int]:
    ...
    Reuses ``security.get_credential_patterns()`` rather than a private regex. That
    accessor exists specifically so a downstream scan cannot be silently turned into a
    no-op by a rename, and it means a pattern added for any other egress site starts
    protecting this one too.
...
async def push(*, message: str = "update ops ledger") -> tuple[bool, str]:
    ...
    leaky = _credential_bearing_lines()
    if leaky:
        logger.error(
