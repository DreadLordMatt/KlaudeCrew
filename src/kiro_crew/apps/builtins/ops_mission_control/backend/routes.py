...
async def _handle_webhook(request: web.Request) -> web.StreamResponse:
    ...
    if raw is None:
        # Reuses ``enqueue``'s own reason string so the status mapping and the audit
        # line are identical whether the cap is hit here or inside ``enqueue``.
        detail = webhook_mod.REJECT_BODY_TOO_LARGE
        _audit("webhook_ingest", detail, "rejected", error=detail)
        return web.json_response(
            {"error": detail, "code": "webhook_rejected"},
