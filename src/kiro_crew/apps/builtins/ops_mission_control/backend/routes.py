...
def _sink_refuses(sink: Any, action: str) -> str:
    ...
    Tolerant of an adapter that does not implement the method, and of one whose
    implementation raises: a companion adapter is third-party code, and a broken
    capability probe must not be able to turn a refusal into a crash. In both cases we
    ALLOW, because the ordinary autonomy gate has already authorized this action and
    ``supported_actions`` is a narrowing hint, not the authority.
    """
    ...
    try:
        supported = probe()
    except Exception:  # noqa: BLE001 — a faulty probe must not break the action path
        logger.exception(
            "ops-mission-control: supported_actions() raised for sink %r",
            getattr(sink, "id", "?"),
        )
        return ""
    if not supported or action in supported:
        return ""
    return (
        f"sink {getattr(sink, 'id', '?')!r} does not support {action!r} "
...
async def _execute_stored_proposal(
    ...
    if result.ok and not result.simulated:
        verification, verify_after = await asyncio.to_thread(
            _schedule_verification, incident.incident_id, action, payload.get("duration_secs")
        )
    return {
        "ok": result.ok,
        "executed": True,
...
async def _handle_put_settings(request: web.Request) -> web.StreamResponse:
    ...
    # VALIDATE BOTH HALVES OF THE AUTHORIZATION PAIR BEFORE WRITING EITHER.
    #
    # `mode` used to be persisted first and the rules validated second, so a request carrying
    # a valid `mode=act` and one malformed rule wrote the mode, returned 400, and left the
    # instance in `act` — activating whatever grants were already stored, from a request the
    # operator was told had FAILED. The two keys are one decision ("may this agent write, and
    # to what"), so a partial application of them is never a safe state. Found in review.
    mode: str | None = None
    if "mode" in body:
    ...
    if "autonomy_rules" in body:
        ...
        ok, code, rules = await asyncio.to_thread(
            rotation.validate_rules, body["autonomy_rules"]
        )
        if not ok:
            return web.json_response(
                {
    ...
    # THE CEILING IS WRITTEN LAST, after EVERY other field has validated.
    #
    # An earlier revision validated `mode` and `autonomy_rules` together and wrote them
    # immediately — atomic with respect to each other, but not to the rest of the request. So a
    # PUT carrying a valid `mode=act` plus an over-long `ledger_sync_remote` (or a
    # credential-bearing remote, or a bad branch ref, or a non-integer tuning value) persisted
    # `act`, THEN returned 400: the operator was told the request failed while the instance had
    # silently begun authorizing provider writes against whatever rules were already stored.
    # Reproduced before fixing. Found in review, one round after the narrower pair-atomicity fix
    # — "validate both halves of the pair" was the wrong scope; the unit is the whole request.
    #
    # Keystone store, not `set_top_level` (which writes the agent-writable config.json): these
    # are the security ceiling, and this authenticated PUT is their sole writer. See
    # `policy_store`.
    if mode is not None:
        await asyncio.to_thread(policy_store.set_mode, mode)
        applied["mode"] = mode
    if rules is not None:
        await asyncio.to_thread(policy_store.set_rules, rules)
...
async def _handle_webhook(request: web.Request) -> web.StreamResponse:
    ...
    if not accepted:
        return web.json_response(
            {"error": detail, "code": "webhook_rejected"}, status=_webhook_reject_status(detail)
        )
