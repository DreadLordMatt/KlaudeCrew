...
async def _execute_stored_proposal(incident: Any, proposal: dict[str, Any]) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {"note": str(proposal.get("note", ""))}
    if action in EXPIRING_ACTIONS:
...
async def _handle_decide_proposal(request: web.Request) -> web.StreamResponse:
    ...
    allowed, reason = rotation.authorize_action(incident.signal, str(proposal["action"]))
    if not allowed:
        return web.json_response(
            {
                "ok": False,
    ...
    result = await _execute_stored_proposal(incident, proposal)
