...
async def _handle_put_settings(request: web.Request) -> web.StreamResponse:
    ...
    # Both validated — now persist. Keystone store, not `set_top_level` (which writes the
    # agent-writable config.json): these are the security ceiling. This authenticated dashboard
    # PUT is the sole writer and reaches the fenced file directly, which is what keeps Settings
    # working while the agent cannot raise its own ceiling. See `policy_store`.
    if mode is not None:
