...
async def _handle_claim(request: web.Request) -> web.StreamResponse:
    ...
    # write.
    firing, _errors = await get_registry().poll_all()
    signal = next((s for s in firing if s.id == claimed_id), None)
    if signal is None:
        return web.json_response(
            {
                "error": (
...
async def _handle_put_settings(request: web.Request) -> web.StreamResponse:
    ...
    if "mode" in body:
        ...
        # Keystone store, not `set_top_level` (which writes the agent-writable config.json):
        # the mode is a security ceiling. This authenticated dashboard PUT is the sole writer
        # and reaches the fenced file directly, which is what keeps Settings working while the
        # agent cannot raise its own ceiling. See `policy_store`.
        await asyncio.to_thread(policy_store.set_mode, mode)
        applied["mode"] = mode

    if "autonomy_rules" in body:
        # The other half of the authorization pair, and it had NO write path — see
        # `rotation.save_rules`. Keystone store for the same reason as `mode`: a rule is what
        # turns `act` into an actual grant, so the agent must not be able to author one.
        #
        ...
        ok, code, normalized = await asyncio.to_thread(
            rotation.save_rules, body["autonomy_rules"]
        )
        if not ok:
            return web.json_response(
                {
        ...
        applied["autonomy_rules"] = normalized
