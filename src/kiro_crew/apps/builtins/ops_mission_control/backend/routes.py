...
def _url_has_userinfo(remote: str) -> bool:
    """True when ``remote`` embeds credentials in its authority (``scheme://user:pw@host``).

    Deliberately narrow, because two legitimate shapes contain an ``@`` and must keep
    working:

    - **scp-style**: ``git@github.com:org/repo.git`` — no ``://``, so the ``@`` is the SSH
      user, not userinfo. This is the single most common remote form there is; flagging it
      would break the recommended setup.
    - **ssh:// with a user**: ``ssh://git@github.com/org/repo.git`` — userinfo, but a
      USERNAME with no secret. SSH authenticates by key, so there is nothing here to leak.

    What is refused is a password-bearing userinfo on an http(s) remote, which is the only
    shape that actually carries a token. Parsed with ``urlsplit`` rather than matched by
    hand: the authority ends at the first ``/``, ``?`` or ``#``, and a hand-rolled ``@``
    search reads a path-embedded ``@`` (``https://host/a@b``) as userinfo.
    """
    ...
    # mistaken for userinfo. A bare `user@` with no password is a username, not a token.
