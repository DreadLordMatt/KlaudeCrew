...
async def _handle_put_settings(request: web.Request) -> web.StreamResponse:
    ...
    if "schedule_strict_gating" in body:
        strict = bool(body["schedule_strict_gating"])

    primary: bool | None = None
    if "primary_instance" in body:
        primary = bool(body["primary_instance"])

    slack_enabled: bool | None = None
    if "slack_enabled" in body:
        slack_enabled = bool(body["slack_enabled"])
    ...
    if primary is not None:
        await asyncio.to_thread(set_top_level, "primary_instance", primary)
    ...
    # The four authorization inputs go to the keystone store, not `set_top_level` (which writes
