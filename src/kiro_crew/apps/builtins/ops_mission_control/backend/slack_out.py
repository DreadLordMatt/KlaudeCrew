...
def _safe(text: str) -> str:
    """The redaction floor for anything Slack-bound. See the module docstring.

    BOTH passes, because neither is a superset of the other: ``redact`` covers AWS keys and
    exfiltration URLs, ``redact_tokens`` covers this app's provider token shapes. Order is
    irrelevant — each only ever replaces a match with a placeholder — but both are required.
    """
...
def _blocks(incident: Incident) -> list[dict]:
    ...
    if incident.signal.url:
        # Through `_safe()` like every other provider-derived field. A URL is not exempt:
        # a signed webhook or console link can carry a token in its query string, and this
        # one is interpolated straight into a Slack block. Missed when `_safe` was
        # introduced for title/resource/detail — the guard test asserted no bare `redact(`
        # remained but not that every FIELD went through it, so this stayed invisible.
        # Found in review.
        context.append(
            {"type": "mrkdwn", "text": f"<{_safe(incident.signal.url)}|open in provider>"}
        )
