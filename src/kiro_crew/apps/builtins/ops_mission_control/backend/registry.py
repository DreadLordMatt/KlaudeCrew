...
class _Truncated(list):
    """A poll result the cap SHORTENED — carried as a type so it cannot be lost.

    A ``list`` subclass rather than a flag threaded through the fan-out: every existing
    consumer keeps working unchanged (``extend``, ``len``, iteration), and the one place that
    needs to know checks ``isinstance``. The alternative — returning a tuple of
    ``(signals, truncated)`` from ``_poll_one`` — would have meant touching the
    ``list | str`` union that distinguishes success from error, where a truncated poll is
    neither.

    Exists because a truncated poll is not a complete snapshot, and absence from a complete
    snapshot is how ``reconcile`` and ``verify_pending_actions`` infer recovery.
    """
        ...
        *,
        ...
        async def _poll_one(src: SignalSource) -> tuple[str, list[Signal] | str]:
            ...
            try:
                ...
                if len(collected) > limit:
                    ...
                    return src.id, _Truncated(collected[:limit])
        ...
        for source_id, outcome in results:
            if isinstance(outcome, str):
                ...
                truncated = isinstance(outcome, _Truncated)
