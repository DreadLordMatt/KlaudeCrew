...
def _channel_inbound_permitted_sync(channel_type: str) -> bool:
    ...
    try:
        ...
        # never drives a turn unaudited. A DENY (any layer) and the ungoverned
        # default-permit (standard OSS build, no policy at all) are logged
        # BEST-EFFORT (``critical=False``): a deny drops the message either way, and
        # OSS availability must not hinge on SEL disk health — but the record is
        # still written so the forensic trail is complete. File-backed SEL, safe in
        # this worker thread.
        layer = getattr(decision, "layer", "")
        governed = layer in ("policy", "profile", "both")
        # EVERY inbound decision leaves a SEL record (the codebase invariant: audit
        # every permission decision — security-deep-dive). Disposition splits on how
        # a persistence failure is handled:
        ...
        #   * DENY, or UNGOVERNED ALLOW (default OSS build, no policy) → best-effort
        #     (critical=False): a deny drops the message either way, and OSS
        #     availability must not hinge on SEL disk health — but the record is
        #     still written so the forensic trail is complete.
        if governed and permitted:
            ...
            # Deny (any layer) OR ungoverned allow: best-effort audit.
            try:
                sel().log_governance_decision(
                    session_key=HOST_SESSION_KEY,
                    tool_name=f"inbound:{channel_type}",
