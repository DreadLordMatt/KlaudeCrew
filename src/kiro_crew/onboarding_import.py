...
_UNSUPPORTED_SCHEDULE_FIELDS = frozenset(
    {
        "command",
        "script",
        "env",
...
_HERMES_UNSUPPORTED_SCHEDULE_FIELDS = (
    _UNSUPPORTED_SCHEDULE_FIELDS
    | frozenset(
        {
            "no_agent",
            "context_from",
...
def _url_has_literal_secret(url: str) -> bool:
    ...
    if parsed.query or parsed.fragment:
        return True
    return any(_SECRET_KEY_RE.search(key) for key, _value in parse_qsl(parsed.query))
...
def _skill_package(
    ...
    for path in _walk_files(package_root, scan, "skills"):
        ...
        if relative.is_absolute() or ".." in relative.parts:
            ...
            return None
        files[relative.as_posix()] = cleaned + ("\n" if text.endswith(("\n", "\r")) else "")
...
def _has_unsupported_schedule_semantics(record: dict[str, Any]) -> bool:
    ...
    if isinstance(schedule, dict) and set(schedule) - _SCHEDULE_SPEC_FIELDS:
        return True
    containers = (record, payload, schedule)
    return any(
        bool(_UNSUPPORTED_SCHEDULE_FIELDS & set(container))
...
def _schedule_from_record(record: Any, scan: _Scan) -> dict[str, Any] | None:
    ...
    if isinstance(schedule, str):
        ...
        timezone_value = schedule.get("timezone", record.get("timezone"))
...
def _hermes_schedule_has_unsupported_semantics(record: dict[str, Any]) -> bool:
    ...
    if fields - _HERMES_SCHEDULE_FIELDS:
        return True
    if fields & _HERMES_UNSUPPORTED_SCHEDULE_FIELDS:
        return True
    ...
    if repeat is not None:
        schedule = record.get("schedule")
        kind = schedule.get("kind", "").casefold() if isinstance(schedule, dict) else ""
...
def _add_markdown_schedules(scan: _Scan, root: Path) -> None:
    for path in _walk_files(root, scan, "schedules", suffixes=(".md", ".markdown")):
        text = _read_text(path, root, scan, "schedules")
        if text is None:
            continue
        metadata, body = _frontmatter(text)
        schedule = metadata.get("schedule", metadata.get("cron", ""))
        record = {
...
def _hermes_roots(scan: _Scan) -> list[Path]:
    ...
    if profiles.is_dir() and not _is_link_like(profiles):
        for child in sorted(profiles.iterdir(), key=lambda path: path.name.casefold())[:50]:
...
def _find_preview_conflicts(scans: list[_Scan]) -> list[dict[str, Any]]:
    conflicts: list[dict[str, Any]] = []
    names: dict[str, set[str]] = {}
    for scan in scans:
        for item in scan.items["mcp_servers"]:
            name = str(item.payload["name"])
            names.setdefault(name, set()).add(scan.source_id)
    for _name, source_ids in sorted(names.items()):
        if len(source_ids) > 1:
            conflicts.append(
                {
...
def _preview(
    ...
    return {
        ...
        "selection": selection,
        "conflicts": _find_preview_conflicts(scans),
        "skipped": skipped,
...
def _write_memory(
    ...
    if isinstance(item.payload, dict) and item.payload.get("kind") == "episodic":
        ...
        existing = vector_store.db.execute(
        ...
        return "existing" if existing is not None else "rejected"
...
def _write_mcp(item: _Item, data_home: Path) -> str:
...
def _write_skill(item: _Item, data_home: Path) -> str:
    ...
    try:
        for relative, content in files.items():
            atomic_write(staging / Path(relative), content)
        if _has_symlink_component(destination, data_home):
            return "rejected"
            ...
            for item in scan.items[category]:
                ...
                try:
                    if category == "sessions":
                        ...
                        status = _write_mcp(item, destination)
