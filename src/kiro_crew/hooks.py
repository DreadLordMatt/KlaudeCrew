...
def _normalize_search_path(value: str) -> str:
    ...
    Steps: expand variables and ``~``, ``normpath``, rewrite separators to ``/``, and
    collapse a leading ``//`` to ``/`` on POSIX. The separator rewrite keeps the
    emitted grammar OS-independent: ``normpath`` produces backslash separators on
    ...
    Never raises: ``expanduser`` raises ``ValueError`` on a ``~name`` form carrying
    an embedded NUL, and this runs inside the permission gate, where an exception is
    a crash rather than a security decision. On any failure the raw value is returned
    for the caller to encode, which cannot forge a field.
    """
    try:
        path = os.path.normpath(os.path.expanduser(os.path.expandvars(value)))
