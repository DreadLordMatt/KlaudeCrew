...
function ChatSidebar({
  ...
  // and the row element the picker popover anchors to (the ⋯ menu closes when
  // the item is picked, so the folder row itself is the stable anchor).
  const [linkingFolderId, setLinkingFolderId] = useState<string | null>(null)
  ...
  const createChatInFolderMutation = useMutation({
    ...
    onSuccess: (slot: Slot, { folderId, columnId }: { folderId: string; columnId?: string }) => {
      if (slot?.key) {
        ...
        // point the new session's project there so skills/prompts resolve
        // against the linked codebase. Best-effort — a failure only means the
        // session starts on the default project.
        const projectDir = resolveFolderProjectDir(folders, folderId)
        // eslint-disable-next-line no-console -- best-effort project link; surface failures for diagnostics
        if (projectDir) api.chatSlotProject(slot.key, projectDir).catch(err => console.warn('Failed to set folder project:', err))
    ...
    return (
      ...
      >
        <div
          ...
          ref={linkingFolderId === folder.id ? linkAnchorRef : undefined}
          ...
          <span className="opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-within:opacity-100 has-[[data-state=open]]:opacity-100 transition-opacity flex items-center gap-0.5">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button type="button" data-testid={`col-${columnId}-folder-${folder.id}-menu`} className="text-muted hover:text-text bg-transparent border-none cursor-pointer p-[2px]" title="More" aria-label={`Folder options for ${folder.name}`} aria-haspopup="menu" onMouseDown={e => { e.stopPropagation() }} onClick={e => { e.stopPropagation() }}>
              ...
              <DropdownMenuContent align="start" className="min-w-[180px]" onClick={e => e.stopPropagation()} onCloseAutoFocus={onMenuCloseAutoFocus}>
                ...
                <DropdownMenuItem onClick={() => { setLinkingFolderId(folder.id) }}><Link2 size={13} /> {folders.find(f => f.id === folder.id)?.project_dir ? 'Change project directory' : 'Link project directory'}</DropdownMenuItem>
        ...
        {linkingFolderId === folder.id && <ProjectPicker open={true} onOpenChange={open => { if (!open) setLinkingFolderId(null) }} anchorRef={linkAnchorRef} onSelect={path => { updateFolderMutation.mutate({ id: folder.id, body: { project_dir: path } }); setLinkingFolderId(null) }} />}
            ...
            <DropdownMenuContent align="start" className="min-w-[180px]" onClick={e => e.stopPropagation()} onCloseAutoFocus={onMenuCloseAutoFocus}>
              ...
              <div className="px-3 py-1.5" onPointerDown={e => e.stopPropagation()}>
                ...
                <input type="text" maxLength={16} placeholder="or type / paste an emoji" aria-label="Custom folder emoji"
                  ...
                  onKeyDown={e => { if (e.key !== 'Enter') return; const v = (e.target as HTMLInputElement).value.trim(); if (!v) return; if (!isSingleEmoji(v)) { setIconErr(true); return } updateFolderMutation.mutate({ id: folder.id, body: { icon: v } }) }} />
