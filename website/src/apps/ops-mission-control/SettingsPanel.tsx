          ...
          {channels.map((ch) => (
            <div key={ch.id} className="flex items-start gap-2 text-[13px]">
              ...
              <dd className="text-muted">
                {CHANNEL_WHEN[ch.id] ?? i18nT('apps.opsMissionControl.settingsPanel.when_a_state_changes')}
    ...
    <div className="flex flex-col gap-4">
      <Card>
        ...
        {rotationQuery.data && rotationQuery.data.rules === 0 && mode === 'act' ? (
          <p className="text-[12px] text-muted mt-1">{i18nT('apps.opsMissionControl.settingsPanel.no_rules_defined_yet')}</p>
        ) : null}
          ...
          <div className="border-t border-border pt-3 mt-3">
            <p className="text-[12px] text-muted">
              {i18nT('apps.opsMissionControl.settingsPanel.companions_installed', {
                noun: companions.length === 1 ? 'adapter package' : 'adapter packages',
