...
/** Prefix of the only message `registry.poll_all` emits for a skipped-because-throttled poll. */
const BACKOFF_PREFIX = 'backing off'
...
/** `BLOCKED_LABEL` with a readable fallback, so an unmapped future reason still prints. */
export function blockedLabel(reason: string | undefined): string {
  if (!reason) return ''
  return BLOCKED_LABEL[reason] ?? reason.replace(/_/g, ' ')
}
...
export function describeSourceHealth(
  ...
  if (err.toLowerCase().startsWith(BACKOFF_PREFIX)) {
  ...
  if (err || entry?.ok === false) {
    return {
      ...
      label: 'failed',
      variant: 'err',
      detail: err || entry?.detail || 'the last poll failed',
      at: entry?.at ?? '',
      contributing: false,
      absenceIsEvidence: false,
    }
...
export function describeVerification(incident: Incident): VerificationView | null {
  ...
  switch (verdict) {
    case 'still_firing':
      return {
        label: 'still firing after this action',
        variant: 'err',
        meaning: `The provider accepted the ${verb} and the signal was still firing when we looked again. The condition this was opened for is live.`,
      }
    case 'cleared':
      return {
        label: 'confirmed',
        variant: 'ok',
        meaning: `We re-read the signal after the ${verb} against a source that answered, and it is no longer firing.`,
      }
    case 'pending':
      return {
        label: 'not checked yet',
        variant: 'muted',
        meaning: `The ${verb} returned success, which only means the provider accepted the request. The next heartbeat after ${incident.verify_after || 'the scheduled time'} re-reads the signal to find out whether it took effect.`,
      }
    case 'unknown':
      return {
        ...
        variant: 'warn',
        meaning: `The re-check was due and the source did not answer, so this proves nothing either way. A later cycle will retry it by itself.`,
      }
    case 'not_checkable':
      return {
        label: 'sent, not confirmed',
        variant: 'warn',
        meaning: `The ${verb} was accepted, but this app cannot observe whether it took effect — an acknowledgement leaves an alert firing by design, so the alarm's state says nothing either way.`,
      }
    default:
      return null
  }
...
export interface RotationInfo {
   ...
   * operator would be showing them KiroCrew's cron plumbing on an ops board, and the
...
export const opsApi = {
   ...
   * not run KiroCrew can be handed, which is why it is worth a round trip.
   *
   ...
   * exists beside the Slack one. Per-channel muting is NOT here: KiroCrew owns that
