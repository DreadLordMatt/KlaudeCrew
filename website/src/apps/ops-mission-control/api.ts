...
export function isKnownBlockedReason(reason: string | undefined): boolean {
  return !!reason && reason in BLOCKED_LABEL_KEY
}

export function blockedLabel(reason: string | undefined): string {
  if (!reason) return ''
  // `i18nT(BLOCKED_LABEL_KEY[reason])`, not `i18nT(key)` via a local: check-i18n-keys
  // resolves element access on an `as const` map (every value is then a checked key) and
  // cannot follow a `let`/`const` indirection, so the local form is an unverifiable site.
  return BLOCKED_LABEL_KEY[reason]
    ? i18nT(BLOCKED_LABEL_KEY[reason])
    : reason.replace(/_/g, ' ')
}
