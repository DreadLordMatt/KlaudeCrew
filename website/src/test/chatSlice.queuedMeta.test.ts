...
import reducer, { appendQueuedMessage, editQueuedMessage, removeQueuedMessage, setActiveSlot, switchSlot } from '../store/chatSlice'
...
describe('hydrated queue cards keep server metadata', () => {
  ...
  //
  // This drives the REAL `switchSlot` thunk against a mocked
  // `api.chatSlotDetail`, because the normalization that carries `q.meta` lives
  // in `fetchSlotDetail`, not in the reducer. Dispatching a hand-built
  // `switchSlot/fulfilled` payload skipped that mapping entirely and passed even
  // with the hydration deleted.
  it('carries meta from the slot-detail payload onto the card', async () => {
    const detail = vi.spyOn(api, 'chatSlotDetail').mockResolvedValue({
      messages: [],
      running: false,
      stopping: false,
      has_more: false,
      total: 0,
      // Server wire shape: `id`, not `queueId`.
      queue: [{ content: `review [attached_dir 1] ${SPACED}`, id: 'q1', meta: { dirs: [SPACED] } }],
    } as never)

    const store = configureStore({ reducer: { chat: reducer } })
    store.dispatch(setActiveSlot(SLOT))
    await store.dispatch(switchSlot(SLOT))

    const next = store.getState().chat
    ...
    expect(card!.meta?.dirs, 'slot-detail meta.dirs never reached the card').toEqual([SPACED])
