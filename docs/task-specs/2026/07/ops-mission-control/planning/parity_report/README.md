...
> ## Status as of 2026-08-02 — read this before trusting a "still open" below
>
> This is a **dated audit snapshot**, deliberately not rewritten as findings close: its
> value is the reasoning and the exercised repro, and editing those in place would destroy
> the record of what was actually true when it was measured. But a report that keeps
> asserting a closed gap misleads, so the ledger of what has since shipped lives here:
>
> | Finding | Status |
> |---|---|
> | §4.2 `write_log` has no caller | **Closed** — called from `store.transition` |
> | §4.4 `needs_human` unsweepable | **Closed** — own longer threshold |
> | §4.5 human opt-out / per-signal mute | **Closed** — `ACTION_SILENCE`, self-expiring |
> | §4.7 ledger-write redaction | **Closed** — redacted before the id is computed, plus a pre-push refusal |
> | §4.8 no `is_primary()` gate on hygiene | **Closed** — route returns 409 `not_primary` |
> | §4.8 non-leader never pulls `rotation.yaml` | **Closed** — `run_cycle` pulls before reading the shift |
> | §4.9 credential expiry / source health | **Closed** — per-source health + consecutive-failure counting |
> | §4.10 handover drops released work | **Closed** — stale bucket restored |
> | §4.11 `claimed_by` provenance | **Closed** — `heartbeat` / `operator`, coerced |
> | §3.2 fingerprint over-merge | **Closed** — exact-identity `provider_key` tier above the shape hash |
> | §4.1 `propose` has no backend | **Closed** — draft store, digest-bound approval, reject, TTL expiry, and a pending queue |
> | §4.3 absence-of-activity as a job class | **Still open** |
> | §4.6 curated knowledge tier / runbooks | **Still open** |
> | §4.8 co-primary `who: [a, b]` hole | **Closed** — warns on a multi-login window |
>
> Two findings in §4 were about *this report's own* accuracy and both were acted on: the
> `features.md` contradictions it names (§4.12) were corrected, and the fingerprint
> collision it reproduced is now covered by a test.
