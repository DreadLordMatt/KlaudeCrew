...
default 0) and the route schedules from that, preferring it over the requested duration.
...
spellings (separate and attached operand) are now matched, and the payload is treated as data by
the verb scan. 24 cases pinned: every `-m`/`-c` spelling blocked, and a payload that imports the
package WITHOUT the mint verb (`python -c "import kiro_crew.cli" && echo ok`) still allowed — the
verb is what the rule blocks, not the import. Found in review (GPT 5.6).
...
\#7 is fenced the same way (`policy_store.PRIMARY_KEY`, written by `PUT /settings`, read by
...
**Seven instances across four review rounds is what per-key tests cost.** Two structural tests now
...
Neither structural test would have caught #7 — the flag is not a provider field and not read by
...
party can write.** #7 also widens it past identity, which the first five all were: the gate it
