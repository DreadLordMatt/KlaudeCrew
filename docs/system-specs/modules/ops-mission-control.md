...
**`companion.companion_summary()` is off-loop for the same reason, on measurement.** It walks
`importlib.metadata.entry_points()`, which enumerates every installed distribution's metadata
from disk, and `/state` is a POLLED endpoint. Measured at ~9.3 ms against ~0.03–0.05 ms for
the sibling reads in the same handler (`store.counts_by_status`, `ledger.stats`) — roughly
200×, with a warm page cache and a small site-packages. Those siblings are deliberately left
inline: each touches one small per-instance JSON file, and a thread hop would cost more than
it saves. The measurement decided the split, not a guess about which calls "look slow".
...
can influence a request URL, and if it can, constrain it to an allowlist at the point of
use rather than trusting the writer.
...
- `.../planning/` — development journals (research sweeps, feature ledger, parity report).
  **Pruned from the sdist** in `MANIFEST.in`: the `recursive-include src/kiro_crew/apps
  *.md` rule exists to ship ATTRIBUTION/README credit and swept these in as a side effect
  (11 files, ~5.2k lines). They stay in git — reviewers read them — but no installed user
  has a use for them and nothing shipped references them. Measured both ways: the sdist
  carried 11 before the prune and 0 after, while the **wheel carried 0 either way**
  (`[options.package_data]` globs `apps/builtins/*/*.md`, a single level, which never
  reached `planning/`), so the review's "ships in the wheel" half does not reproduce.
  In-package `tests/` is deliberately NOT pruned — `code_review_sage` and `issue_radar`
  ship theirs, so that is the convention for a builtin app.
