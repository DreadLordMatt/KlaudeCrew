...
non-integer tuning value — still persisted `act` and *then* hit one of those later 400s. The
unit of atomicity is the whole request: `mode`/`autonomy_rules` are the only fields whose
partial application grants authority, so they are now the LAST writes in the handler, after
every validation. Reproduced before fixing; a test drives all five rejection paths and asserts
`app_mode()` stays `observe`, and fails against the old ordering. Found in review both times.
