...
`_authorize` also runs the synchronous gate through `asyncio.to_thread`. Its off-shift check
resolves this instance's GitHub identity by spawning `gh api user` (10s timeout) when
`schedule-file.github_login` is unset, which is the documented default; the two handlers
that reach the gate never `await registry.resolve_shift()` first, so nothing has warmed the
login cache off-loop. Run inline, the first such request in a fresh gateway froze the event
loop — the user's chat turn and the liveness heartbeat with it. Guarded behaviourally: a
test ticks a heartbeat coroutine during the request and fails if the loop stalls.
