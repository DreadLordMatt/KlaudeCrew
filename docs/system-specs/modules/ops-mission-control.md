...
`"import sys; …; from kiro_crew.cli import main"` to `import sys` and hid the import entirely. 28
cases pinned, 15 blocked and 13 allowed, including `python -c 'print(1)' token` and
`grep -r kiro_crew src/` to show the deny is scoped to code we are about to run. 24 cases pinned: every `-m`/`-c` spelling blocked, and a payload that imports the
