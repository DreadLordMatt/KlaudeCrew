...
Provider config the agent legitimately reads (regions, prefixes, channel ids) stays in
`config.json`; only the ceiling moved. A test pins the policy path onto `is_sensitive_path`
and the filename equal to the fence entry.
