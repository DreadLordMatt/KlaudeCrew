...
\#6 is fenced the same way (`policy_store.PRIMARY_KEY`, written by `PUT /settings`, read by
...
**Six instances in three review rounds is what per-key tests cost.** Two structural tests now
...
Neither structural test would have caught #6 — the flag is not a provider field and not read by
...
party can write.** #6 also widens it past identity, which the first five all were: the gate it
