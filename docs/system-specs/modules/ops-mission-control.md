...
**Both halves of the pair are validated before EITHER is written.** `mode` used to be
persisted first and the rules validated second, so a request carrying a valid `mode=act` and one
malformed rule wrote the mode, returned 400, and left the instance in `act` — activating
whatever grants were already stored, from a request the operator was told had FAILED. `mode` and
`autonomy_rules` are one decision ("may this agent write, and to what"), so a partial
application is never a safe state. `rotation.validate_rules` was split out of `save_rules` so
the route can check everything first; both paths share the same validation code, so they cannot
disagree. Found in review.
...
dispatches it through `asyncio.to_thread`, so each source's sync core (`resolve_now`,
`pagerduty._on_shift_sync`) runs off the loop; making this async would push the await up
through the whole gate for nothing. A source offering *only* the coroutine abstains rather than
being driven from a worker thread — abstaining is already the documented fail-open behaviour.
...
spooled for the next cycle. Each individual deque op holds the GIL for its whole duration, so
no single step interleaves.
