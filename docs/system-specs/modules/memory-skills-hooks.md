...
unsupported store. Before any supported foreign SQLite database is opened, the
main file and present `-wal`/`-shm` sidecars must all be regular non-symlink
...
Accepted packages retain their ordinary assets.
...
MCP import is merge-only. Existing server definitions win on name/collision,
and KiroCrew-managed servers (including `kirocrew-core` and `kirocrew-cron`) are
protected from replacement, deletion, or shadowing by an imported definition.
Repeated imports deduplicate through the provenance ledger.
