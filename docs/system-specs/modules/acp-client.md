...
`AcpClient(acp_backend=...)` selects which subprocess to launch. Fork
(KlaudeCrew): the value is config-driven — `agent.acp_backend` (enum
...
otherwise the value is passed through **unresolved** and
`_apply_startup_model` resolves it against the session's own just-captured
...
## Turn usage (claude backend)

`TurnUsage` (`acp/types.py`) is a per-turn billing snapshot carried on `AcpEvent(EVENT_COMPLETE)`; kiro fills only `credits` (from `_kiro.dev/metadata` `meteringUsage`, see below), claude fills token counts + `cost_usd`. **Fork (KlaudeCrew), source-verified** against the pinned `claude-agent-acp` v0.66.0 package (`dist/acp-agent.js`, the same version this doc already cites for the model wire shape above) and then **live-confirmed** on a real gateway (v0.66.0 adapter + native `claude` 2.1.227): both signals below arrived with the traced shapes, e.g. a prompt result carrying `usage.{inputTokens:2, outputTokens:4, cachedReadTokens:48675, cachedWriteTokens:823}` and a `usage_update` carrying `cost:{amount:0.0326775, currency:"USD"}`. Two independent, differently-shaped signals exist:

1. **`PromptResponse.usage`** (`acp/_dispatch.parse_prompt_result_usage`) — an EXPERIMENTAL/optional ACP-spec field (`usage?: Usage | null`) that claude-agent-acp's `prompt()` handler resolves to `{stopReason, usage: sessionUsage(session)}` for **every** stop reason (`end_turn`, `refusal`, `cancelled`), sourced from `session.accumulatedUsage` — which the adapter zeroes both at session creation and whenever a queued turn is promoted to active, so these counts are **per-turn**, never cumulative. The one exception: a queued-then-cancelled turn that never ran settles with `{stopReason: "cancelled"}` and **no `usage` key at all** — `parse_prompt_result_usage` returns `None` for that shape (and for any result where every recognized field is unusable), and callers must read that as "no data for this turn", not zero. Field names are camelCase (`inputTokens`, `outputTokens`, `cachedReadTokens`, `cachedWriteTokens`, `totalTokens`); KlaudeCrew's `cache_creation_tokens` maps to `cachedWriteTokens`. Populated at the ONE `_dispatch_events` "complete" arm that has a real JSON-RPC result to read (`AcpClient` in `acp/client.py`) — the two synthesized-completion sites (kiro's tool-interrupted-marker and stale-turn timeout) have no result to parse from and leave the token fields at whatever `last_prompt_stats` already carries (0, on claude, since only the real result populates them).
2. **`usage_update.cost`** (`acp/_dispatch.parse_usage_update_cost`) — an optional `Cost` sub-object (`{amount, currency}`) on the SAME `usage_update` notification `_track_usage_update` already parses for the context-window ring meter (`used`/`size`), sourced from the Claude Code SDK's own running `total_cost_usd`. The ACP spec's own docstring for `Cost.amount` is explicit: **"Total cumulative cost for session"** — NOT a per-turn delta. `AcpClient._track_usage_update_cost` diffs successive reads against `self._cost_usd_baseline` (an instance field that survives `AcpPromptStats.carry_over()`'s per-turn reset, since it is the baseline the NEXT delta is computed against, not a per-turn value itself) and adds only the positive delta to `last_prompt_stats.cost_usd`. The baseline starts at **0.0**, not at the first observed value: the adapter's accumulator is **per process** — live-confirmed by driving `session/load` against a session with ≥$0.33 of prior spend and observing the first post-load `usage_update` report `cost.amount ≈ 0.03` (this process's own first turn), not a continuation of the prior total. So the first cumulative value seen IS the first turn's spend and must be credited; seeding the baseline from it would silently drop that turn's cost on every fresh process and every resume. A negative delta (the SDK's own tracker resetting under it) resyncs the baseline without subtracting from an already-accumulated total. Also live-observed: `session/load` replays no `usage_update` before the next prompt, so a resumed session's context-ring stays at its pre-resume reading until the first turn completes.

Both signals combine into the same `EVENT_COMPLETE`'s `TurnUsage` via `turn_usage_from_stats(stats)` (`acp/types.py`) — the single constructor for all three `_dispatch_events` `EVENT_COMPLETE` sites (replacing the old `TurnUsage(credits=...)` literal at each), so the sites cannot drift from each other, and also used by `llm_helpers.provider_last_turn_usage` (background surfaces that lose the event object) via defensive `getattr` rather than this stricter constructor, since that function's `provider` is `Any` and not guaranteed to expose a real `AcpPromptStats`. `TurnUsage.num_turns`/`duration_ms` stay 0 on both backends — the ACP wire carries neither.

Testing: `src/kiro_crew/testing/fake_acp_backend.py`'s `[[USAGE]]` trigger (claude dialect only) emits both signals with fixed, assertable values, mirroring the `[[THOUGHT]]` trigger's dialect-gating pattern. Unit/end-to-end coverage: `test/test_klaude_turn_usage.py`.
