...
`TurnUsage` (`acp/types.py`) is a per-turn billing snapshot carried on `AcpEvent(EVENT_COMPLETE)`; kiro fills only `credits` (from `_kiro.dev/metadata` `meteringUsage`, see below), claude fills token counts + `cost_usd`. **Fork (KlaudeCrew), source-verified** against the pinned `claude-agent-acp` v0.66.0 package (`dist/acp-agent.js`, the same version this doc already cites for the model wire shape above) — not a live gateway capture; the exact JSON has not been eyeballed off a running turn, only traced through the adapter's own source and the ACP SDK's `types.gen.d.ts`. Two independent, differently-shaped signals exist:
...
2. **`usage_update.cost`** (`acp/_dispatch.parse_usage_update_cost`) — an optional `Cost` sub-object (`{amount, currency}`) on the SAME `usage_update` notification `_track_usage_update` already parses for the context-window ring meter (`used`/`size`), sourced from the Claude Code SDK's own running `total_cost_usd`. The ACP spec's own docstring for `Cost.amount` is explicit: **"Total cumulative cost for session"** — NOT a per-turn delta. `AcpClient._track_usage_update_cost` diffs successive reads against `self._cost_usd_baseline` (an instance field that survives `AcpPromptStats.carry_over()`'s per-turn reset, since it is the baseline the NEXT delta is computed against, not a per-turn value itself) and adds only the positive delta to `last_prompt_stats.cost_usd`. The first observation in a process has nothing to diff against, so it seeds the baseline without attributing any cost to that turn (rather than guess how much of a resumed session's prior spend belongs to "now"); a negative delta (the SDK's own tracker resetting under it) resyncs the baseline without subtracting from an already-accumulated total. **Residual live-verification risk**: whether `total_cost_usd` truly resets to 0 on a fresh SDK process vs. carrying forward real prior spend across a `session/load` resume has not been confirmed live — if it does NOT reset, the very first turn after a resume under-attributes cost (baseline-seed, no delta), which is a silent undercount, not a crash or a wrong-turn misattribution.
