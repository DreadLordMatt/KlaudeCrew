...
**Probe failure classification + cache policy (2026-07-18 incident)**: `detect_backend()` caches a positive result (`namespace`/`sandbox-exec`) for the process lifetime, but `none` is cached **only when the userns probe failure is permanent** (kernel refuses user namespaces: EPERM/EINVAL/ENOSYS, propagated from the forked probe child via its exit code). Transient resource failures (`_TRANSIENT_PROBE_ERRNOS`: fork EAGAIN/ENOMEM/EMFILE/ENFILE/ENOSPC) are logged with their errno, retried once in-probe (`_PROBE_TRANSIENT_RETRY_DELAY_SECS`), and **never cached** — the next spawn re-probes and self-heals. All non-`off` modes share one cache entry (backend capability is mode-independent), and `config_mode="off"` short-circuits without probing or touching the cache. The fail-closed `RuntimeError` and its SEL `denied` event include the probe failure reason; for transient failures the message says to retry, NOT to set `sandbox_allow_unsandboxed_exec`. Rationale: one EAGAIN during a cron spawn burst previously poisoned the cache and fail-closed every spawn for ~1h until gateway restart.

**Event-loop safety — never-block-on-loop invariant**: The event loop NEVER executes fork/waitpid/sleep for the sandbox probe. `prewarm_backend()` is called at boot (both `slack/gateway.py` and `mcp_gateway/gatewayd.py`) to populate the cache before any on-loop caller reaches `detect_backend()`. If an on-loop caller encounters a cold cache (race at startup or after `reset_backend()`), `_probe_unshare()` fires `_kick_background_warm()` (a daemon thread that runs the probe off-loop and fills the cache) and returns False with a transient reason — `detect_backend()` returns `"none"` (uncached), `wrap_argv()` raises the fail-closed `RuntimeError` with retry guidance, and the caller retries on the next spawn (typically within ms, by which time the background thread has filled the cache). This eliminates the prior bounded-wait approach (executor + `future.result(timeout)`) which still blocked the loop for up to 2–4s.
...
- **Raw-command evaluation (upstream [1f4ea490](https://code.amazon.com/packages/MeshClaw/commits/1f4ea490a740459e769e7d78d14f84d01d6bc269) / [CR-290333917](https://code.amazon.com/reviews/CR-290333917))** — the title for a shell tool may be an **LLM-authored `description`** rather than the literal command (`select_tool_title` in `acp/_dispatch.py` prefers `description` over `command`), so it is untrusted for security decisions: `Bash(command="cat ~/.aws/credentials", description="check my config")` would otherwise be checked as "check my config". Callers pass the raw command via the keyword-only `command=` (sourced from `AcpEvent.shell_command`, which recovers it from `raw_tool_params` OR the `tool_input` JSON of a permission event); every security check (`is_sensitive_path`, `is_sensitive_bash_command`, `audit_bash_exfiltration`, the PolicyAuthority deny list) then runs against the title AND the command — a match on **either** denies (over-blocking is the safe direction). Auto-approve stays keyed on the title only, so a failure to auto-approve merely falls through to interactive approval. All enforcing call sites thread `command=`/`is_shell=`: chat_runner, llm_helpers, Slack handler (x2), subagent, task_executor, task_planner, and the fork's slack/telegram/wechat `transport_dispatch` `_tool_gate` closures.
- **Deny-by-default for unrecoverable shell commands** — when a caller reports `is_shell=True` but no `command` could be recovered, the call is DENIED rather than evaluated on the untrusted title alone (the title-only fallback would be the very bypass this gate closes). Non-shell tools (`is_shell=False`, no command) are still gated by title as before. The Slack `EVENT_TOOL_CALL` path is informational (the tool already executes and cannot be rejected there), so deny-by-default is not armed on it; a deny-list match there is surfaced as "flagged (already executing)" with SEL outcome `flagged_unenforceable`, and the LLM-authored title is redacted (`redact_exfiltration_urls` + `redact_credentials`) before posting.
