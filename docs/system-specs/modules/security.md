...
**Nested-sandbox passthrough**: when `wrap_argv()` is called from a process that is *already inside* a KiroCrew sandbox (script-cron ticks, sandboxed agent children, app backends, pooled MCP servers), it returns the argv unchanged (one-shot info log) instead of trying to wrap again. Nested sandboxing is impossible on **both** backends: the Linux launcher's seccomp-BPF filter denies `unshare`/`setns`, and macOS Seatbelt refuses `sandbox_apply` with `Operation not permitted` from inside an existing sandbox — even under an `(allow default)` outer profile. Without the passthrough the fail-closed `RuntimeError` above bricks **every** in-sandbox spawn: on Linux each in-sandbox MCP spawn (the probe error was raised on each `ctx.call_tool` and silently swallowed by the caller), and on macOS ~40 `MCP probe failed` entries at gateway boot plus app backends unable to start (`app 'dev-fleet' has no reachable backend`) and Dev Fleet / Files `git` calls failing with "No OS-level sandbox backend is available on this host" — a claim that is **false** on a host whose `sandbox-exec` works unnested. This is **not** a fail-open path: the outer sandbox still confines every descendant, so passthrough spawns within the existing isolation boundary.

Detection requires **two independent facts that cover each other's blind spot**:

1. **The marker** (`_active_sandbox_tier()` / `_IN_SANDBOX_MARKER`) proves KiroCrew built the outer sandbox *and* scrubbed the credential environment on the way in. It is exported at exactly two sites, each immediately after that platform's env scrub: the Linux launcher `main()` (beside `KIROCREW_HOST_PID`, after the `ENV_PREFIXES` loop) and the macOS `env` prefix built by `sandbox_exec_argv()` (after its `env -u` flags, which are derived from the **same** `_SENSITIVE_ENV_PREFIXES` / `_AGENT_DENIED_ENV_KEYS` / `_PYTHON_ENV_PREFIXES` logic as the Linux launcher — see `_sandbox_env_unset_args()`). A marked process therefore always has an environment KiroCrew already sanitised, which is what makes passthrough safe for callers that use `wrap_argv()` directly rather than `sandboxed_spawn_argv()`. The gate deliberately does **not** key on `KIROCREW_HOST_PID` — that variable is dual-purpose session-identity plumbing, and gating a security-relevant passthrough on a variable set for unrelated reasons would be a latent bypass. No unsandboxed code path sets the marker, and `cli.main()` drops any inherited value.
2. **The kernel** (`_macos_sandbox_state()`, macOS only) independently confirms a sandbox is active via `sandbox_check(pid, NULL, SANDBOX_FILTER_NONE)`. This is OS-authoritative and cannot be spoofed through an agent-influenced environment, but it cannot identify *whose* profile is active, so it can never grant passthrough on its own.

The kernel check is **tri-state** rather than boolean because the two negatives carry opposite meanings. A definite `False` (kernel says: not sandboxed) alongside a present marker proves the marker was forged or inherited into an unconfined process, and **vetoes** the passthrough with a `SECURITY:` warning. `None` (symbol unavailable, ABI change, restricted dyld, or non-darwin) says nothing at all, and must **not** retroactively invalidate a marker the Linux path honours unconditionally. The state is `lru_cache`d — a process cannot leave its sandbox.

The passthrough is SEL-audited on **every** invocation via `log_tool_invocation(outcome="allowed", metadata={"reason": "nested_sandbox_passthrough", ...}, critical=True)`, mirroring the `denied` event on the fail-closed path so the security decision is tamper-evidently recorded. `critical=True` gives it the same write reliability as the `denied`/`delegated` audits — the event is written **synchronously** after draining the async backlog, so a slow or wedged background writer cannot silently drop passthrough records. It stops short of full audit-or-deny (re-raise on SEL failure) deliberately: unlike `_delegate_to_kiro_internal_sandbox` — which on audit failure falls back to KiroCrew's own seatbelt, an equally-safe audited layer — a nested passthrough has **no** safe alternative (the re-wrap is impossible by design), so failing the spawn on a SEL filesystem error would couple every in-sandbox call to SEL health and reintroduce a prior in-sandbox spawn outage. On a hard write failure it therefore logs loudly and proceeds: the child is confined by the outer sandbox whether or not the record lands.

**Foreign outer sandbox is NOT passed through**: reaching the fail-closed branch while the kernel reports this process *is* sandboxed means the confiner is one KiroCrew did **not** build — a KiroCrew-built sandbox carries the marker and was already passed through before backend detection ran. The remaining case is kiro-cli >= 2.13's own internal seatbelt (see the mutual-exclusion rule above) or an operator-wrapped gateway. Those are refused, because macOS exposes no supported way to identify *which* profile is active: the path-scoped `sandbox_check` form that could prove the outer profile denies credential reads is variadic and returns `-1` through ctypes on arm64, and a settings file saying kiro's internal sandbox is *enabled* is a configuration claim, not evidence about the active profile. So an outer profile permissive enough to leave `~/.aws` / `~/.ssh` readable cannot be ruled out, and the env scrub that makes the marker case safe may never have run. What the nesting detection *does* fix here is the **diagnosis**: `_inside_macos_sandbox()` lets the error say "this host's sandbox is not broken, you are nested under a sandbox KiroCrew did not create" and point at the config-level remedy — `{"sandbox": false}` in `~/.kiro/settings/amazon-internal.json`, which hands isolation back to KiroCrew's own profile (the one that hides the credential paths, so this **keeps** isolation rather than weakening it) — instead of the previous claim that the host has no sandbox backend, which sent operators hunting for a missing backend that was never missing. It deliberately does not steer them to `sandbox_allow_unsandboxed_exec`, which would disable isolation everywhere to fix a case where a sandbox demonstrably exists.

**A nested passthrough may not downgrade the isolation tier**: the marker's **value** is the tier the outer sandbox was built at (`standard` / `cc` / `strict`; builds predating this wrote a bare `"1"`, read back as `"unknown"`). A passthrough necessarily runs at the *outer* sandbox's tier — it cannot tighten a sandbox it is already inside — so when `_tier_downgrade()` shows the request is **stricter** than the host sandbox, `wrap_argv()` **refuses** with a `RuntimeError` naming both tiers, and `_nested_downgrade_denied()` emits a `denied` SEL event (`reason: nested_sandbox_tier_downgrade_denied`) mirroring the no-backend fail-closed path. Unlike the passthrough — which has no safe alternative and therefore proceeds on a SEL write failure — the refusal *does* have one, so a SEL outage never turns it into an allow. Silently passing through would hand repo- or agent-influenced code the credential directories that `standard` deliberately exposes, for a caller that explicitly asked for them to be hidden.

This closes a **pre-existing** gap on both platforms, not just macOS: the marker check used to sit above the `sandbox_level` derivation and was therefore tier-blind, so an in-sandbox `strict` request silently ran at the host sandbox's tier. The places in the tree that hit it were all in Dev Fleet, which runs inside an app backend spawned at `standard` (`apps/backend.py`) and asked for `strict` in six spawns: the rebase pair, the sync runner's ff-merge / pip / npm steps, and pod provision. None of them ever received `strict`. They now declare `standard` honestly. The rebase additionally carries an explicit `repo_controlled=True` flag for the property `mode="strict"` had been doing double duty for in `_run_cmd` — withholding the operator's git credential helpers from an invocation that executes worktree content; the sync and provision steps already gated credentials on `_build_env()` / `_pod_env()` rather than the tier, so only the tier changed there. The primary control is unchanged throughout: `_GIT_ENV_NEUTRALIZERS` pins `core.hooksPath` to `/dev/null` and clears `core.fsmonitor` and `credential.helper`. A static AST guard (`test_no_spawn_requests_a_tier_stricter_than_this_backend_runs_at`) now fails the build if any `"cc"`/`"strict"` literal reappears in that module — a keyword-only search for `mode="strict"` misses the positional `sandboxed_spawn_argv(argv, "strict")` form, which is how five of the six were written.

The opposite direction is **allowed and common** — a `standard` spawn from inside a `cc` script-cron sandbox already has more isolation than it asked for. `_tier_downgrade()` only claims what it can prove: an unrecognised tier on either side yields `False`, because "cannot compare" is not "is weaker", so a legacy `"1"` marker still passes through. Both tiers are recorded on the `allowed` event (`active_tier`, `requested_tier`) so an auditor can see the isolation a spawn actually ran under rather than infer it.
