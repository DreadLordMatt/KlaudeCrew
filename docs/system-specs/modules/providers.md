...
- **Resume guard:** `session/load` (resume) is only attempted when the prior session transcript exists on disk (`~/.kiro/sessions/cli/<sid>.json`). A stale persisted sid with no transcript falls back to `session/new`, preventing a fresh conversation from replaying old turns (which inflated base context). Fork (KlaudeCrew): the claude backend gets the same guard via a getattr hook (`AcpClient._claude_session_transcript_exists`, attached by `klaude/registry.py` to `klaude/transcripts.claude_transcript_exists`, which checks the SDK's own `~/.claude/projects/<munged-cwd>/<sid>.jsonl`). Absent (public core) it defaults to attempting the load, unchanged from before the hook existed; on any lookup error it also fails toward attempting the load (never toward silently skipping a real resume).
- **Session cleanup:** `AcpProvider.cleanup_session()` deletes the kiro transcript pair on the kiro backend; fork (KlaudeCrew) attaches `AcpClient._claude_cleanup_transcript` (same registry, `klaude/transcripts.delete_claude_transcript`) for the claude backend's SDK transcript. Both fail soft — a delete failure logs a warning and never propagates.
