...
- **Unprimed** (`loaded` is False): the first load may **not** be skipped, so a
  caller that finds the lock held **waits** for it. This happens at most once per
  process, and the holder is performing exactly the load the waiter needs, so it
  cannot wedge the loop. Concurrent first-touch is the *expected* case, not an
  exotic one: nothing primes the store on the ungoverned / profile-only boot path
  (`assert_profiles_within_ceiling` early-returns when no ceiling is present), so a
  startup burst across the five transports puts several `mc-gov` threads here at
  once. Regression-locked by
  `test_cold_store_contention_never_serves_ungoverned_permit`.
- **Warm** (already loaded): **non-blocking** `acquire(blocking=False)`. A caller
  that can't grab the lock serves the current immutable snapshot and self-heals on
  the next access. Non-blocking is mandatory here because `_ensure_fresh` is
  reachable on the event loop (the synchronous PreToolUse gate), where waiting on
  another thread's filesystem I/O would wedge the loop; it is safe because `_snap`
  is only ever replaced wholesale (an atomic ref swap), so a concurrent reader sees
  a coherent prior-or-next snapshot — and because a prior snapshot *exists*, the
  worst case is authorizing against the last committed state for one call.
...
warm caller reuses its pre-lock fingerprint rather than re-statting under the lock
(a second walk on the loop would be a slow-FS stall for no freshness gain), and
only a caller that actually *waited* re-stats — its pre-lock view may have gone
stale while the winner published. Either way, the fingerprint used for the
freshness test is the one committed, so the committed fingerprint always describes
the snapshot actually published.
