...
| [powers](modules/powers.md) | Kiro Powers: inert `POWER.md` bundle browse / install / remove, atomic stage-rollback-commit installer, provenance-only `installed.json` schema, registry mirror (official monorepo + kiro.dev marketplace), `fetch.py` bundle-download security controls. Activation — trust, enable/disable, MCP registration, generated skills — is deliberately deferred |
