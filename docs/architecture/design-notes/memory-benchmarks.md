...
> **These figures are being re-measured and should not be quoted yet.** They are means
> over 1 977 queries, and that population included all **446 LoCoMo adversarial items**
> — every one of which is `unanswerable` and carries gold evidence, so every one was
> scored even though the correct behaviour for them is *refusal*. Rewarding retrieval
> there counts 22.6% of the population with the sign flipped. The harness now excludes
> them (`BenchQuery.scorable_retrieval`), which leaves **1 531** scorable queries, and a
> corrected run of both arms is in flight. The shape of the decay result below is not in
> doubt — it is a 5–6× effect measured on identical populations in both arms — but the
> absolute values will move.

Measured over all 1 977 then-scorable LoCoMo queries with the **real
Qwen3-Embedding-0.6B model** (`qwen3-embedding:0.6b@1024`, `sqlite_cosine` backend,
`null_embeddings = 0` in both arms), same corpus / ranker / embedder, the only
difference being whether the decay term is allowed to act:

| | `--timeline now` (decay neutral) | `--timeline anchored` (real gaps) |
|---|---|---|
| strict session recall@1 | 0.4942 (1 977 queries) | **0.0814** (1 977 queries) |
| strict session recall@3 | 0.6555 (1 977 queries) | 0.1388 (1 484 queries) |
| strict session recall@5 | 0.7365 (1 977 queries) | 0.3288 (**73** queries) |
| strict turn recall@5 | 0.4112 (1 977 queries) | **0.0754** (1 977 queries) |
| queries where @5 is measurable | 1 977 / 1 977 | **73 / 1 977** |
| queries where @10 is measurable | 1 807 / 1 977 | 0 |

Read `recall@1` and `turn recall@5` first: those are the two cut-offs measurable on
the **identical 1 977 queries** in both arms, so they are the only rows where the
difference is attributable to the decay rather than to a changed population. They
fall 0.4942 → 0.0814 and 0.4112 → 0.0754 — the decay removes roughly **five sixths**
